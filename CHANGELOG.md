...
- **A scanned page whose text layer is a JBIG2 soft mask rendered inverted, and any soft-masked artwork washed out as it shrank** — two independent defects on the image `/SMask` path. The JBIG2 decoder expands straight to 8-bit gray, taking a branch that skips the packed-sample block where `/Decode` is otherwise applied, so the array was honoured for every other filter and silently dropped for this one; ISO 32000-1:2008 §8.9.5.2 Table 90 applies to a JBIG2 image like any other, and Table 145 explicitly permits `/Decode` on a soft-mask image — it constrains only the default to `[0 1]`, unlike the entries it marks "Ignored" — while §11.6.5.3 requires the alpha be derived with the Decode transformation already performed. Scanners write `/Decode [1 0]` to normalise whichever polarity their encoder emitted, so dropping it inverted the mask exactly: the page painted its background through the glyphs and hid everything else. Separately, the SIMD downscale un-premultiplied its input. The blit premultiplies before resampling, which is the correct order, but `fast_image_resize`'s `use_alpha` defaults to "premultiply, resize, un-premultiply", and over an already-premultiplied buffer that divides the alpha back out — handing the compositor straight-alpha RGB in a buffer it reads as premultiplied, so `dst = src + dst×(1−a)` adds the colour at full strength over a backdrop it should only have tinted. A uniform alpha round-trips through that unharmed, which is why it stayed hidden until a soft mask appeared; a masked image drifted lighter the more it shrank, by 4.5 grey levels at 0.75 scale and 12.7 at 0.267, against reference renderers that are scale-invariant (#1200, #1217).
- **Two labels stacked one line apart ran together into a single word** — `same_line_threshold` allows 1.2x the *smaller* of two font sizes so that ordinary leading does not manufacture line breaks, which for two spans of the same size admits everything below 1.2 em, a plain single-spaced advance included. A wrapped body line is still separated further down by the large negative `delta_x` it gets from restarting at the left margin, but a stacked table-header label starts at or to the right of the line above, so its horizontal gap is near zero and no arm on that branch separated the pair: the forward-gap guard wants more than 1.25 em, the font-transition arm more than 0.5 pt, and the generic space threshold sees nothing. A results-table header reading `Latency` above `Efficiency` extracted as `LatencyEfficiency`. Spans whose baselines differ by at least 0.8 em of the larger font now break the line when the gap is too small for the existing guard — 0.8 em sits below any real line advance and well above a superscript or subscript shift of about 0.3 em, which must stay attached to the token it decorates (ISO 32000-1:2008 §9.4.3: the show operators paint the glyphs they are given, so two words drawn on different baselines are two words) (#1220).
- **A chart legend that draws its labels over one another extracted them as one word** — two runs painted at the same place have baselines a fraction of a thousandth of a point apart. The leaf sort used to compare baselines with an exact float equality, which dropped the two labels into separate groups and left each in draw order; the row-aware comparator that replaced it — so that jittered OCR baselines keep their reading order — collapses that difference into one band and interleaves the labels by x. The assembler then met a pair restarting within half an em of the previous run's origin while overlapping its ink by four em, and every arm of its negative-gap chain declined: the full-line-back arm wants 20 em, the column-return arm wants a delta of −3 em, the baseline-drop arm wants half an em of drop against the 8.6×10⁻⁴ pt actually present, the wrap arm wants 1.0, the fonts are equal, and the inflated-width arm wants 1.5 em. Nothing separated them and a results legend reading `maximum` over `minimum` extracted as `maximumminimum`. A span restarting within half an em of its predecessor's origin while overlapping by more than three em is now a second line (ISO 32000-1:2008 §9.4.3 — the show operators paint the glyphs they are given, so two runs painted over one another are two runs). The rule is strictly additive, reached only where every existing arm already declined: a superscript always advances forward, the widest overlap treated as reliable kerning is one em, and a fraction denominator sits about two em back (#1220).
- **A clipping path made of a single move-to was discarded, letting an unbounded shading flood the page** — §8.5.3.3.1: "A single-point open subpath (specified by a trailing m operator) shall produce no output", and §8.5.4 sets the clip to the *intersection* of the current clip with the new path, which for an empty region is empty. The rasteriser's path builder rejects a lone move-to, and reading that as "no clip was asked for" left the previous, page-sized clip in force; a following `sh` with `/Extend [true true]` then painted DeviceN black across the whole sheet (Table 77). A scanned colour TV schedule rendered as a near-black rectangle. Deleting the page's image changed not one pixel — the artwork was being painted over. Across the corpus 66 documents carry such a clip; honouring it leaves 64 byte-identical and moves two toward the reference renderers (#1223).
- **A run routed to the per-glyph fallback rendered vertically mirrored** — §9.4.4 makes the text-state/text-matrix/CTM product the whole glyph-space-to-device transform, and the page's single y-flip already lives in the page base transform. The fallback branch negated the y scale on top of that; it was the only negative scale in the text pipeline, so anything reaching it came out upside-down (#1227).
- **An uncoloured tiling pattern painted grey instead of its colour** — §8.7.3.3 and Table 74: for a `PaintType 2` pattern the cell carries no colour and the operands preceding the pattern name in `scn` supply it, in the underlying space named as the second element of the `[/Pattern base]` array. With no arm for `Pattern`, the array fell through to a grey fallback reading the first operand as a grey level, so `0 0 1 /P scn` — pure blue — became black and the stencil poured grey (#1228).
- **A graphics state leaked between regions of a large content stream** — above 256 KB the text extractor replays only the `BT..ET` regions, injecting each region's absolute CTM and bracketing it so that one region's state cannot reach the next. A stream may open a `q` it closes only at the end of the page, and CAD exporters routinely do; when that `q` fell inside a replayed region, the region's own `Q` popped it instead of the injected save and the CTM stayed on the stack. On a 621 KB drawing 0.12 became 0.12², so 52 pt type was reported at 0.75 pt, and the assembler's absolute font-size clamps then disabled every word separator. §8.4.2: `Q` restores the state saved by the matching `q`, and a `Q` whose `q` was never replayed has none. Wrong text-rendering matrices on that page went from 137 of 208 show operations before this release, and 201 of 208 during it, to zero (#1226).
- **A widget's border stroked pale where the file asks for solid** — the appearance-to-`/Rect` fit also enabled the `/BBox` clip. Table 95 does say those boundaries "shall be used to clip the form XObject", but §12.5.5 scales the BBox to fill `/Rect`, so anything stroked *on* the boundary is half outside by construction. Widget appearances routinely are: a date field's `1 w 0 G 0 0 149.9998 22 re S` inside a `/BBox [0 0 150 22]` lost half its stroke. The fit stays; the clip is off again for appearance streams (#1235).
- **A right-to-left word was split into single letters** — the converters treat a span ending before the previous one begins as a reading discontinuity, which stops a re-ordered OCR layer emitting `theisIt`. That premise is left-to-right: in Arabic and Hebrew a continuation steps leftward by definition, so every glyph pair inside a word satisfied it and a heading reading القهوة came out as ال ق هو ة. §14.8.2.3.3 describes right-to-left show-text runs, whose glyphs advance the other way. The rule now applies only where neither side carries a right-to-left character (#1236).
- **A superscript footnote marker was pushed onto its own line** — the stacked-lines rule broke the line at 0.8 em of baseline separation. Measured, the pair that reaches the assembler is a 7.04 pt comma and a 7.04 pt marker 5.78 pt apart, which is 0.82 em: the marker's own run is small, so the same physical shift is a far larger fraction of it than the ~0.3 em a superscript costs against body text. Raised to one em, which is below any real line advance (normal leading is ~1.2) and above this shift (#1237).
- **A rotated marginal stamp was emitted inside a body sentence** — the row-band sort compares baselines along page-y and orders within a band along page-x, and both keys assume a shared writing axis. §9.4.4: a glyph's displacement is interpreted in text space, so a run at 90° advances along a different page axis. An arXiv sidebar stamp whose baseline fell 0.43 pt from a body line's banded with it and, its origin being near the page edge, sorted to the front — with no separator, because the gap test subtracted its along-axis advance from a horizontal x. The sort now keys on the writing-axis quadrant first (#1238).
- **A soft hyphen was stripped even where it renders as a visible hyphen** — Annex D note 5: the hyphen "is also encoded as 255 in WinAnsiEncoding … the meaning of this duplicate code shall be 'soft hyphen,' but it shall be typographically the same as hyphen". Removing it unconditionally deleted characters the reader sees: a part number `SS<shy>2541<shy>03<shy>M` became `SS254103M` and a date `2023<shy>06<shy>15` became `20230615`, both of which poppler and MuPDF keep. It also fired on a 0xAD byte that was half a GBK sequence, and on a maths font mapping U+00AD to a large parenthesis. §14.8.2.2.3 licenses removing an incidental artifact of layout, but only where the soft hyphen is one — a letter on both sides. Deciding that per span turned out to be impossible: the span writer sees `wonder<shy>` and `ful` as separate fragments and cannot tell a word wrapped across two lines from a marker it must keep, and the space that stands in for the line break is added later still. The wrap is therefore closed on assembled text, where both sides are finally visible, so a word broken across a line rejoins with no space while a 0xAD that is not surrounded by letters is left exactly where the file put it. Whether the halves belong together depends on what separates them, and the seam can only answer that for fragments with **nothing** between them — contiguous glyphs, which are one word whatever follows. Whitespace means the assembler already decided from the span rectangles that these were separate runs, and a function that sees only characters has no standing to overrule it: `prin<shy>` + `from` at a column break is textually identical to `wonder<shy>` + `ful` at a line wrap. Guessing cost 464 words against poppler on one OCR'd newspaper, where a wrap ending one column swallowed the start of the next. The genuine wrap is still closed a level up, on assembled text, where the line structure is visible (#1239, #1250, #1253).
- **The `s` operator was not mapped, so close-and-stroke paths were never painted** — Table 60: "s — Close and stroke the path. This operator shall have the same effect as the sequence h S." The paths-only parser decomposed it, but the main operator table, which the streaming parser and therefore every annotation appearance stream goes through, had no entry at all. AcroForm field borders are drawn exactly this way, so they vanished across seven documents while a sibling button drawn with `f` rendered fine (#1229).
- **A column-merge cap moved borderline pages between table and prose in both directions** — the cap scaling the merge threshold to a table's own pitch was applied unconditionally, so once it split a page's columns that set was the only one tried: it either survived the emission gates or the page became prose. Across the corpus that moved 255 table pages to prose and 59 the other way, the same mechanism in both directions. It is now probed like the neighbouring refinement — adopted only when the grid it produces still passes every gate — and its pitch is measured on the columns that recur down the page rather than on greedy per-word clusters, which on an indented index numbered 13 where the table has 4 (#1241).
